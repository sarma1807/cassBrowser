...
function check_os_compat() {
    ...
    if [ "$os" != "Linux" ]; then
        ...
        case "$os" in
            Darwin)
                echo "Detected : macOS"
                echo "This script is for Ubuntu-compatible Linux only."
    ...
    if [ ! -f /etc/os-release ]; then
        ...
        echo ""
        echo "This script requires an Ubuntu-compatible Linux distribution."
        echo ""
        exit 1
    fi
    ...
    if [ $is_ubuntu_family -eq 0 ] && echo "$id_like" | grep -qE '(ubuntu|debian)'; then
        is_ubuntu_family=1
    fi

    if [ $is_ubuntu_family -eq 0 ]; then
        print_error "Unsupported Linux distribution : ${pretty_name:-$distro_id}"
        echo ""
        echo "This script is designed for Ubuntu-compatible distributions :"
        echo "  - Ubuntu"
        echo "  - Debian"
        echo "  - Linux Mint"
        echo "  - Pop!_OS"
        ...
        echo ""
        echo "The package manager commands (apt-get, dpkg) and repository"
    ...
    if ! command -v dpkg &>/dev/null; then
        print_error "dpkg command not found"
        echo ""
...
function check_go() {
    if ! command -v go &> /dev/null; then
        ...
        echo ""
        echo "Note : Ubuntu's packaged Go may be older than the latest release."
        echo "       For the latest Go version, use the official installer :"
...
function check_build_reqs() {
    ...
    # libstdc++-dev is a virtual package on newer Ubuntu — check for any installed version
    if ! dpkg -l 'libstdc++-*-dev' 2>/dev/null | grep -q '^ii'; then
        local libstdcxx_pkg
        libstdcxx_pkg=$(apt-cache search '^libstdc\+\+-[0-9]+-dev$' 2>/dev/null | sort -t- -k3 -n | tail -1 | awk '{print $1}')
        missing_pkgs+=("${libstdcxx_pkg:-libstdc++-14-dev}")
        has_issues=1
    fi
    ...
    if [ $has_issues -eq 1 ]; then
...

        {
            ...
            if [ $need_go -eq 1 ] && [ ${#missing_pkgs[@]} -gt 0 ]; then
                echo "apt-get install -y golang-go ${missing_pkgs[*]}"
...
function build() {
    ...
    if go build -v -ldflags "-X main.AppName=$BINARY_NAME -X 'main.AppDisplayName=$APP_DISPLAY' -X main.AppVersion=$APP_VER -X main.AppVersionDate=$APP_DATE -X main.EncryptConnectionDetails=$ENCRYPT_CONN" -o "$BINARY_NAME" "$BUILD_DIR"; then
        ...
        BUILD_INFO_FILE="${BINARY_NAME}_BuildInfo_Ubuntu.txt"
...
function clean() {
    ...
    if [ -f "${BINARY_NAME}_BuildInfo_Ubuntu.txt" ]; then
        echo "Removing build info file : ${BINARY_NAME}_BuildInfo_Ubuntu.txt"
        if rm -f "${BINARY_NAME}_BuildInfo_Ubuntu.txt"; then
            cleaned=1
        else
            print_error "Failed to remove ${BINARY_NAME}_BuildInfo_Ubuntu.txt"
            return 1
        fi
    fi
